...
현재 주 데이터와 4주 평균을 가중 평균하여 예측:
```
현재 시간당 평균 = 현재 주 총 트윗 / 경과 시간
4주 평균 시간당 = 4주 평균 총 트윗 / 경과 시간

가중 평균 시간당 = (현재 시간당 × 0.7) + (4주 평균 시간당 × 0.3)
다음 24시간 예측 = 가중 평균 시간당 × 24
```
**가중치**: 현재 주 70%, 4주 평균 30% (constants.js에서 조정 가능)

#### End of Range (범위 종료 시 예상)
현재 데이터와 남은 시간 예측을 합산:
```
현재 시간당 평균 = 현재 주 총 트윗 / 경과 시간
4주 평균 시간당 = 4주 평균 총 트윗 / 경과 시간

가중 평균 시간당 = (현재 시간당 × 0.7) + (4주 평균 시간당 × 0.3)
남은 시간 예측 = 가중 평균 시간당 × 남은 시간
최종 예상 = 현재까지 총합 + 남은 시간 예측
```

#### Trend (트렌드 지표)
현재 주와 4주 평균을 비교하여 활동 추세 표시 (예측에는 미사용):
```
...
- ↘️ 하락: < 0.85 (15% 이상 감소)
```
**참고**: 트렌드는 시각적 표시용이며, 예측 계산에는 가중 평균 방식 사용
